...
function setupEventListeners() {
    ...
    if (addRoomBtn) {
        console.log('Found add room button, adding event listener');
    ...
    if (importRoomsBtn) {
        console.log('Found import rooms button, adding event listener');
        importRoomsBtn.addEventListener('click', openImportModal);
    } else {
        console.log('Import rooms button not found');
    }
    
    const exportStudentsBtn = document.getElementById('exportRoomsBtn');
    if (exportStudentsBtn) {
        console.log('Found export rooms button, adding event listener');
        exportStudentsBtn.addEventListener('click', exportRooms);
    } else {
        console.log('Export rooms button not found');
    }
    
    const clearAllRoomBtn = document.getElementById('clearAllRoomBtn');
    if (clearAllRoomBtn) {
        console.log('Found clear all rooms button, adding event listener');
        clearAllRoomBtn.addEventListener('click', clearAllRoomsAndNotify);
    } else {
        console.log('Clear all rooms button not found');
    }
    
    // Form submission
    const roomForm = document.getElementById('roomForm');
    if (roomForm) {
        console.log('Found room form, adding submit event listener');
        roomForm.addEventListener('submit', handleRoomSubmit);
    } else {
        console.log('Room form not found');
    }
    
    const importForm = document.getElementById('importForm');
    if (importForm) {
        console.log('Found import form, adding submit event listener');
    ...
    if (roomSearch) {
        console.log('Found room search input, adding input event listener');
        roomSearch.addEventListener('input', debounce(loadRooms, 300));
    } else {
        console.log('Room search input not found');
    }
    ...
    if (closeImportModalBtn) {
        console.log('Found close import modal button, adding event listener');
        closeImportModalBtn.addEventListener('click', closeImportModal);
    } else {
        console.log('Close import modal button not found');
    }
    
    // Cancel buttons
    const cancelImportBtn = document.getElementById('cancelImportBtn');
    if (cancelImportBtn) {
        console.log('Found cancel import button, adding event listener');
        cancelImportBtn.addEventListener('click', closeImportModal);
    } else {
        console.log('Cancel import button not found');
    }
    
    // Import submit button
    const importSubmitBtn = document.getElementById('importSubmitBtn');
    if (importSubmitBtn) {
        console.log('Found import submit button, adding event listener');
        importSubmitBtn.addEventListener('click', function(e) {
            console.log('Import submit button clicked');
            e.preventDefault();
            const importForm = document.getElementById('importForm');
            if (importForm) {
                console.log('Dispatching submit event to import form');
        ...
        console.log('Import submit button not found');
    }
    
    // Close modal when clicking outside
    console.log('Adding window click event listener for closing modals');
    ...
    // Add roll assignment event listeners
    console.log('Setting up roll assignment event listeners');
            ...
            tableBody.innerHTML = `
                <tr>
                    <td colspan="5" style="text-align: center; padding: 2rem; color: var(--error-600);">
                        ...
                        <p>Failed to load room assignments: ${error.message || 'Unknown error occurred'}</p>
                    </td>
                </tr>
            `;
        }
...
function renderRollAssignmentsTable(assignments) {
    ...
    assignments.forEach(assignment => {
        ...
        const rollNumbersHtml = assignment.rollNumbers && assignment.rollNumbers.length > 0 
            ? assignment.rollNumbers
                .map(rollNo => `<span class="roll-number-tag">${rollNo}</span>`)
                .join('')
            : '<span class="no-assignments">No roll numbers assigned</span>';

        html += `
            <tr>
                <td>${assignment.number || 'N/A'}</td>
                <td>${assignment.building || 'N/A'}</td>
                <td>${assignment.capacity || 0}</td>
                <td>${assignment.rollNumbers ? assignment.rollNumbers.length : 0}</td>
...
async function handleImportSubmit(event) {
    ...
    try {
        ...
        // Directly get the file from the input element instead of using FormData
        ...
        const overwriteCheckbox = document.getElementById('overwriteData');
        const overwriteData = overwriteCheckbox ? overwriteCheckbox.checked : false;
        console.log('Overwrite data:', overwriteData);
        
        const { data } = await parseCSV(file);
        console.log('Parsed CSV data:', data);
        
        if (data.length === 0) {
            console.log('No valid rooms found in the file');
            showToast('No valid rooms found in the file', 'error');
            return;
        }
        
        // Transform data to match API expectations
        const rooms = data.map(row => ({
            number: (row['number'] || row['Number'] || row['room_number'] || row['Room Number'] || '').toString().trim(),
            building: (row['building'] || row['Building'] || row['location'] || row['Location'] || '').toString().trim(),
            capacity: parseInt(row['capacity'] || row['Capacity'] || 0) || 0
        }));
        console.log('Transformed rooms data:', rooms);
        ...
        if (validRooms.length === 0) {
            console.log('No valid room data found in CSV');
            showToast('No valid room data found in CSV', 'error');
            return;
        }
        ...
        console.log('Adding rooms to database');
        // Add rooms to the database using bulk import
        await fetchData('rooms/bulk', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        console.log('Closing import modal and loading rooms');
        closeImportModal();
        loadRooms();
        showToast(`Rooms imported successfully (${validRooms.length} rooms)`, 'success');
    } catch (error) {
